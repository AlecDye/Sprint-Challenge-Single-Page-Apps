...
> Answer: The primary advantage of using client-side routing is that a single page app doesn't need to reload the entire page when the user interacts with it.  For example, in server-side routing, if the user submits a form that would cause the entire page to reload even though they're only using a small part of the entire page. Client-side routing would only update the form without refreshing the page and all of its assets.
